...
const AdminDashboard = () => {
  ...
  const handleLogout = () => {
    dispatch(logout());
    dispatch(setCurrentView('landing')); // Redirect to landing page after logout
  };
  ...
  /* Sidebar Nav Item Helper - FIXED: Added dispatch */
  const NavItem = ({ icon: Icon, label, view, active }) => (
    <button
      onClick={() => { if (view) dispatch(setCurrentView(view)); setSidebarOpen(false); }}
            ...
            <div className="flex space-x-2 overflow-x-auto pb-2 mb-6">
              {['pending', 'approved', 'rejected', 'all'].map(view => (
                <button
                  ...
                  className={`px-4 py-2 rounded-xl text-sm font-bold transition whitespace-nowrap ${currentViewFilter === view
                  ...
                  <div key={doctor.user_id} className="bg-white rounded-2xl p-6 border border-gray-100 hover:shadow-lg transition flex flex-col md:flex-row md:items-center justify-between gap-6 group">
                    ...
                    <div className="flex items-center gap-4 self-end md:self-auto">
                      ...
                      <div className="flex flex-col gap-1">
                        <button
                          onClick={() => handleBlockUser(doctor.user_id, 'doctor')}
                          className="px-3 py-1.5 bg-rose-50 text-rose-600 rounded-lg font-bold text-xs hover:bg-rose-100 transition"
                        >
                          Block
                        </button>
                        <button
                          onClick={() => handleUnblockUser(doctor.user_id, 'doctor')}
                          className="px-3 py-1.5 bg-emerald-50 text-emerald-600 rounded-lg font-bold text-xs hover:bg-emerald-100 transition"
                        >
                          Unblock
                        </button>
                      </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm transition-opacity">
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col animate-in fade-in zoom-in-95 duration-200">
              ...
              <div className="flex-1 overflow-y-auto p-8 space-y-8">
                {/* Status Banner */}
                <div className={`p-4 rounded-xl border ${selectedDoctor.doctor?.doctor_status === 'approved'
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className={`font-bold ${selectedDoctor.doctor?.doctor_status === 'approved' ? 'text-emerald-900' :
