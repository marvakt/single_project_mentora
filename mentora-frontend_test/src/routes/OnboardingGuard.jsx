...
export default function OnboardingGuard({ children }) {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      try {
        // 2️⃣ Profile must exist
        const res = await getProfile(user_id);
        const profile = res.data;
        const onboarding = profile.onboarding_status ?? 0;

        // 3️⃣ USER onboarding
        if (role === "user" && onboarding < 100) {
          return setRedirect(`/onboarding/user/${user_id}`);
        }

        // 4️⃣ DOCTOR onboarding
        if (role === "doctor") {
          if (onboarding < 50)
            return setRedirect(`/onboarding/doctor/basic/${user_id}`);
          if (onboarding < 80)
            return setRedirect(`/onboarding/doctor/docs/${user_id}`);
          if (onboarding < 100)
            return setRedirect("/onboarding/doctor/pending");
        }
        ...
        console.error("OnboardingGuard failed:", err);

        // ANY failure → logout (no guessing)
